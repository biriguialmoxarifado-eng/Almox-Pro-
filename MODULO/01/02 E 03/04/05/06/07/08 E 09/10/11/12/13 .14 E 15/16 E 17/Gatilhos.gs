...
/** Instala os seis triggers de tempo de uma vez (rodar manualmente uma vez). */
function setup_instalarGatilhosDeTempo() {
  ScriptApp.getProjectTriggers().forEach(t => {
    if (['Gatilho_VerificarEstoqueCritico', 'Gatilho_VerificarReservasVencendo', 'Gatilho_VerificarNiveisEstoque', 'Gatilho_GerarInventarioD1', 'Gatilho_VerificarVistoriasPendentes', 'Gatilho_DiagnosticoAutomatico'].includes(t.getHandlerFunction())) {
  ...
  SpreadsheetApp.getUi().alert('Gatilhos instalados: estoque crítico (6h), reservas vencendo (1h), níveis de estoque (1x/dia), inventário D-1 (1x/dia às 20h), vistorias pendentes (1x/dia às 7h), diagnóstico automático (1x/dia às 5h).');
}
