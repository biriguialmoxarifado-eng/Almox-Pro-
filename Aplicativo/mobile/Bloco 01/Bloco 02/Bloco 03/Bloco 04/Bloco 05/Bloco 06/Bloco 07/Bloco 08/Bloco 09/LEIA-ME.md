...
# BLOCO 06 (Módulo de Ferramentas) — JÁ INCLUÍDO

Relatório desta rodada ANEXADO ao `MODULO06_FERRAMENTAS_RELATORIO.md`
já existente (mesmo nome pedido pela spec) — nada apagado.
Ferramentas já estava construído e funcional; 4 gaps reais
preenchidos: **prazo real de retirada + detecção de atraso**
(o evento `FERRAMENTA_ATRASADA` existia reservado desde sempre,
nunca tinha dado pra calcular), **estado `BLOQUEADA`** (mais
severo que `COM_PROBLEMA`, reservado pra não conformidade
crítica, sem quebrar o caminho já testado pra gravidade ALTA),
**troca de ferramenta** (nunca existia), e **disponibilidade
agregada**. Também corrigi 2 eventos que o módulo dizia emitir
mas não emitia de verdade.

## Como instalar

1. Cole os arquivos alterados: `Service_Ferramenta.gs`, `DB_Mapping.gs`, `Core_Constants.gs`, `Core_Config.gs`, `Event_Types.gs`, `Notificacao_Events.gs`, `Gatilhos.gs`, `TESTS/Test_IntegracaoFinal.gs`.
2. Adicione `Test_Bloco06_Ferramentas.gs` em `/TESTS` e rode.
3. Rode `setup_instalar` de novo (2 colunas novas em FERRAMENTAS, tabela nova FERRAMENTA_TROCAS).
4. Rode `setup_instalarGatilhosDeTempo` de novo (instala o gatilho de ferramentas atrasadas).

**Bloco 06 de 8** — próximos (07 e 08) em rodadas futuras.
...
13 fases de backend + 9 fases de Front Mobile + 17 módulos de
negócio/arquitetura + 3 integrações estruturais + FRONT-B01 +
BLOCO 02 + BLOCO 03 + BLOCO 04 + BLOCO 05 + BLOCO 06 = 216
arquivos `.gs`, zero erros de sintaxe.
---

# BLOCO 07 (Relatórios) — JÁ INCLUÍDO

Leia `BLOCO07_RELATORIO.md`. Achado importante: já existia uma
infraestrutura de relatórios extensa (Fase 9 — 13 tipos, exportação
real em PDF/CSV/Google Sheets). Preenchidos 5 builders novos
(Ocorrências, Pré-Compras, Inventário Detalhado, Valor
Inventariado, Itens por PEP, Rastreabilidade), permissão financeira
real (só GESTOR/AUDITOR/ADMIN acessam relatório com dado
financeiro) e escopo de obra (MESTRE_OBRA só vê a própria obra).
Front ampliado com os 6 cards pedidos + filtros + exportação.

## Como instalar

1. Cole os arquivos alterados: `Service_Relatorio.gs`, `API_Relatorios.gs`, `Screen_Reports.html`, `TESTS/Test_IntegracaoFinal.gs`.
2. Adicione `Test_Bloco07_Relatorios.gs` em `/TESTS` e rode.
3. Nenhuma tabela nova — nenhum `setup_instalar` necessário.

**Bloco 07 de 8** — falta só o Bloco 08 pra fechar os 8 blocos de
Frontend anunciados.
...
13 fases de backend + 9 fases de Front Mobile + 17 módulos de
negócio/arquitetura + 3 integrações estruturais + FRONT-B01 +
BLOCO 02 + BLOCO 03 + BLOCO 04 + BLOCO 05 + BLOCO 06 + BLOCO 07 =
217 arquivos `.gs`, zero erros de sintaxe.
---

# BLOCO 08 (Etiquetas/QR Code) — JÁ INCLUÍDO — ÚLTIMO DOS 8 BLOCOS

Leia `BLOCO08_RELATORIO.md`. O backend deste bloco (modelos, lote,
ZPL, leitura de QR) já estava completo quando esta entrega
começou. Completei a única lacuna real: a tela não expunha
modelos salvos nem geração em lote, mesmo o backend já suportando
os dois — adicionado.

## Como instalar

1. Cole o arquivo alterado: `Screen_Labels.html`.
2. Todo o resto (`Service_Etiqueta.gs`, `API_Etiquetas.gs`, `MOD_23_ETIQUETAS.gs`, teste) já estava no zip.
3. Nenhuma tabela nova — nenhum `setup_instalar` necessário.
...
FRONT-B01, BLOCO 02, 03, 04, 05, 06, 07, 08 — todos concluídos e
testados. Somados às 13 fases de backend, 9 fases de Front Mobile,
17 módulos de negócio/arquitetura e 3 integrações estruturais:
**218 arquivos de backend, zero erros de sintaxe.**

## Para instalar o sistema inteiro do zero

1. Crie uma planilha Google Sheets nova.
2. Extensões → Apps Script.
3. Cole todos os arquivos `.gs` do zip (um arquivo por `.gs`, a pasta de origem não importa dentro do Apps Script).
4. Cole os arquivos `.html` da pasta `FRONTEND/`.
5. Rode `setup_instalar()` — cria todas as abas com as colunas certas.
6. Rode `setup_instalarGatilhosDeTempo()` — instala os gatilhos automáticos.
7. Configure as chaves de `Core_Config` que dependem de você (`DRIVE_FOLDER_BACKUP`, `DRIVE_FOLDER_DOCS`, etc).
8. Implante como Web App (Implantar → Nova implantação → App da Web).
9. Rode `Test_RunTudo()` pra confirmar tudo funcionando junto.
---

# BLOCO 09 (Prancha de Conexão / Central de Integração) — JÁ INCLUÍDO

Leia `BLOCO09_RELATORIO.md`. Bloco extra, fora dos 8 anunciados.
A regra mais importante do contrato ("não inventar conexões") só
foi possível cumprir de verdade depois de **ampliar o `Event_Bus`**
— ele nunca soube dizer qual módulo era dono de cada listener, e
sem isso qualquer linha no mapa seria uma suposição. Corrigido
primeiro (retrocompatível), depois construído o mapa real (SVG em
desktop/tablet, lista em mobile) e a síntese no backend
(`Doctor_IntegrationMap`, zero lógica de negócio, só reúne dado
que já existia).

## Como instalar

1. Cole os arquivos alterados: `Event_Bus.gs`, `Service_PreCompra.gs`, `Event_Core.gs`, `Audit_Events.gs`, `Notificacao_Events.gs`, `Doctor_Core.gs`, `FRONTEND/JS/App.html`, `Front_App.html`, `Screen_Settings.html`, `TESTS/Test_IntegracaoFinal.gs`.
2. Adicione os arquivos novos: `Doctor_IntegrationMap.gs`, `FRONTEND/Screen_ConnectionBoard.html`.
3. Adicione `Test_Bloco09_PranchaDeConexao.gs` em `/TESTS` e rode.
4. Nenhuma tabela nova — nenhum `setup_instalar` necessário.
...
13 fases de backend + 9 fases de Front Mobile + 17 módulos de
negócio/arquitetura + 3 integrações estruturais + FRONT-B01 +
BLOCO 02 a 09 (8 blocos de Frontend anunciados + 1 bloco extra de
arquitetura) = 220 arquivos `.gs`, zero erros de sintaxe.
