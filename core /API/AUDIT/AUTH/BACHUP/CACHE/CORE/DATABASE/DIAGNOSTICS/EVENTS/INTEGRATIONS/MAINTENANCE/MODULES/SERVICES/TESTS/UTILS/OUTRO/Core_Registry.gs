...
const Core_Registry = (function () {
  ...
  let _publicRoutes = {}; // FASE 2 (Front Mobile): ações que não exigem sessão
                          // (a lojinha precisa ser navegável sem login — seção 1
                          // da spec do front: "a pessoa não deve ser obrigada a
                          // entrar no sistema completo simplesmente pra solicitar
                          // um material"). Só o módulo dono da rota pode registrar
                          // isso no próprio init() — nunca é decisão do Front.

  function registerPublicRoute(action) {
    _publicRoutes[action] = true;
  }

  function isPublicRoute(action) {
    return action === 'auth.login' || !!_publicRoutes[action];
  }
  ...
  function reset() { _modules = {}; _routes = {}; _publicRoutes = {}; } // uso exclusivo em testes

  return { registerModule, registerRoute, registerPublicRoute, isPublicRoute, getModule, getAllModules, getRoute, getAllRoutes, reset };
})();
