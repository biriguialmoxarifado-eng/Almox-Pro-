...
/**
 * BLOCO 06 — ferramenta atrasada nunca tinha como ser detectada
 * (retirar() não gravava prazo). Mesmo padrão dos outros gatilhos.
 */
function Gatilho_VerificarFerramentasAtrasadas() {
  Core_API.bootstrap();
  const ctx = Core_Context.build({ action: 'sistema.gatilho', userId: 'sistema' });
  const resultado = Service_Ferramenta.verificarFerramentasAtrasadas(ctx);
  Logger.log('Gatilho ferramentas atrasadas: ' + resultado.total + ' encontrada(s).');
  return resultado;
}

/** Instala os oito triggers de tempo de uma vez (rodar manualmente uma vez). */
function setup_instalarGatilhosDeTempo() {
  ScriptApp.getProjectTriggers().forEach(t => {
    if (['Gatilho_VerificarEstoqueCritico', 'Gatilho_VerificarReservasVencendo', 'Gatilho_VerificarNiveisEstoque', 'Gatilho_GerarInventarioD1', 'Gatilho_VerificarVistoriasPendentes', 'Gatilho_DiagnosticoAutomatico', 'Gatilho_BackupAutomatico', 'Gatilho_VerificarFerramentasAtrasadas'].includes(t.getHandlerFunction())) {
  ...
  ScriptApp.newTrigger('Gatilho_VerificarFerramentasAtrasadas').timeBased().everyHours(2).create();
  SpreadsheetApp.getUi().alert('Gatilhos instalados: estoque crítico (6h), reservas vencendo (1h), níveis de estoque (1x/dia), inventário D-1 (1x/dia às 20h), vistorias pendentes (1x/dia às 7h), diagnóstico automático (1x/dia às 5h), backup automático (1x/dia às 3h), ferramentas atrasadas (a cada 2h).');
}
